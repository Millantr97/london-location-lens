"""Extract relevant POIs from Greater London OSM pbf into a pool JSON - FULL GL bbox (wave 2)."""
import osmium, json
...
class H(osmium.SimpleHandler):
    ...
    def node(self,n):
        ...
        if not (BBOX[0]<=la<=BBOX[2] and BBOX[1]<=lo<=BBOX[3]): return
        self.out.append({'type':'node','id':n.id,'lat':round(la,6),'lon':round(lo,6),'tags':{k:v for k,v in t}})
    def way(self,w):
        t=w.tags
...
print('nodes:',len(h.out),'ways:',len(h.ways),flush=True)
WANT=set()
...
json.dump(h.out,open('/tmp/lens/pipeline/cache/pool_full.json','w'))
print('pool_full:',len(h.out),flush=True)
